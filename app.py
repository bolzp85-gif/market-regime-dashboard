...
CFTC_HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (compatible; QuantRegimeDashboard/3.0)"
    ),
...
def fetch_option_put_call(
    ...
    """
    PCR = Put-Volumen / Call-Volumen.

    Wir aggregieren die nächsten bis zu drei verfügbaren
    Laufzeiten, statt nur die erste Laufzeit zu verwenden.
    Das reduziert die Zufälligkeit einer einzelnen Expiry.

    Wichtig:
    Bei Futures wird kein "Futures-P/C" behauptet.
    ES/NQ/GC/CL/6E werden über SPY/QQQ/GLD/USO/FXE
    als Optionsproxy abgebildet.
    """
    try:
        t = yf.Ticker(ticker)
        ...
        if not expiries:
            return np.nan, False, "No option chain"
        ...
        return (
            float(ratio),
            True,
            f"{ticker} Options ({len(used)} Expiries)"
        )
    ...
    if FRED_API_KEY:
        try:
            ...
            if (
                ...
                # WALCL and WTREGEN are in millions USD.
                # RRPONTSYD is in billions USD.
                # Result is expressed in billions USD.
                df["net_liquidity"] = (
                    wal / 1000
                    - tga / 1000
                    - rrp
...
def fetch_sp500_pe_history():
    ...
    headers = {
        "User-Agent": (
            "Mozilla/5.0 "
            "(compatible; QuantRegimeDashboard/3.0)"
        )
    }
...
st.caption(
    ...
    "Die Data Coverage zeigt, wie viel der aktiv gewichteten "
