...
st.set_page_config(
    page_title="Multi-Asset Regime Dashboard v4",
    page_icon="📊",
    layout="centered"
)
...
def fetch_google_trends_sentiment(asset_name):
    ...
    try:
        ...
        # High score = more search interest in "fear" terms.
        score = float(
            np.clip(
                50 - latest * 15,
                0,
...
def normalize_to_percentile(
    ...
    """
    Rolling z-score -> Normalverteilung -> 0..100.

    Wichtig:
    Fehlende Reihen bleiben intern NaN und werden erst bei der
    Säulenberechnung neutral mit 50 behandelt. Dadurch wird ein
    Offline-Feed nicht fälschlich als echte Live-Messung dargestellt.
    """
...
def calculate_mci(scores, weights, coverage=None):
    """
    Model Consistency Index (0..100).

    Wichtig:
    - Fehlende Komponenten dürfen nicht als echte 50er-Werte eine
      perfekte Konsistenz vortäuschen.
    - Wenn Coverage mitgegeben wird, werden nur tatsächlich verfügbare
      Säulen in die Konsistenzberechnung einbezogen.
    - Bei weniger als zwei verwertbaren Säulen ist der MCI bewusst auf
      50 begrenzt: Mit nur einer Säule kann man keine echte
      Modellkonsistenz beurteilen.
    """
    ...
    # Effective weight = model weight x actual data coverage.
    ...
    if np.isclose(sd, 0.0, atol=1e-9):
        # Identical real observations are consistent; identical 50s
        # are only neutral, not evidence of a perfect model.
...
def build_pillar_score(
    ...
    """
    Fehlende Komponenten werden nicht als "echte 50" gezählt.
    Die verfügbare Gewichtung wird renormalisiert.
    Zusätzlich wird die Datenabdeckung je Säule gemessen.
    """
    cols = [
        c for c in weights
    ...
    for idx in raw_df.index:
        ...
        if values and available_weight > 0:
            score.at[idx] = float(
                np.average(
                    values,
                    weights=w
...
def calculate_model_confidence(
    ...
    """
    Datenqualität statt scheinbarer Sicherheit:
    - yFinance ist zwingend
    - FRED ist für Makro aktiv
    - COT ist für Positionierung aktiv
    - CNN ist Sentiment-Zusatz
    - PCR ist Zusatz
    - Bewertung ist nur für S&P aktiv
    - aktuelle Model_Data_Coverage reduziert die ausgewiesene Sicherheit
    """
    checks = []
    ...
    # Sentiment and PCR are supplemental, not hard requirements.
    supplemental = [
    ...
    supplement = (
        sum(supplemental) / len(supplemental)
    ) * 25.0

    # Coverage is a separate quality constraint. Confidence must not
    # remain high merely because missing components are neutralized.
    latest_coverage = 0.0
    if (
        df is not None
        and not df.empty
...
# ============================================================
# 6. CFTC COT – ROBUSTE JAHRESDATEIEN
# ============================================================

CFTC_HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (compatible; QuantRegimeDashboard/4.0)"
    ),
    "Accept": "application/zip,application/octet-stream,*/*"
}


def _read_cftc_zip_year(year):
    urls = [
        (
            "https://www.cftc.gov/files/dera/history/"
    ...
    for url in urls:
        try:
            ...
            with zipfile.ZipFile(
                ...
                if not members:
                    last_error = "ZIP empty"
                    continue

                # Usually one CSV/TXT file is contained.
                member = members[0]
...
def _parse_cftc_date_series(series):
    """Parse the different date formats used by CFTC historical files."""
    ...
    # MM/DD/YYYY and generic final fallback
    mask = result.isna()
    result.loc[mask] = pd.to_datetime(
        raw.loc[mask],
        format='%m/%d/%Y',
...
def fetch_cot_data(asset_search_string):
    """Robust CFTC Commercial Net Position series for the selected asset."""
    year = pd.Timestamp.now().year
    frames = []

    # Two years are sufficient for the current 756-day normalization
    # window and keep the download small. Try current + previous year.
    for yr in [year - 1, year]:
        d = _read_cftc_zip_year(yr)

        if d is None or d.empty:
            continue

        market_col = _find_cftc_column(
            d,
            [
    ...
    try:
        ...
        d['Date'] = _parse_cftc_date_series(d[date_col])
        ...
        d = (
            d.dropna(subset=['Date', 'Net_Commercials'])
        ...
        series = d.set_index('Date')['Net_Commercials']
        return series, True
...
def fetch_option_put_call(ticker, max_expiries=3):
    """
    Put/Call volume ratio from the selected ETF options proxy.

    Futures do not have a directly used "futures P/C" here. The model
    explicitly uses ETF option proxies: ES->SPY, NQ->QQQ, Gold->GLD,
    WTI->USO, EUR/USD->FXE.
    """
    try:
        t = yf.Ticker(ticker)

        # yfinance versions differ in how Ticker.options behaves.
        try:
        ...
        try:
            expiries = list(raw_expiries)
        except TypeError:
            expiries = []

        expiries = [
            str(x) for x in expiries
        ...
        # Defensive date filtering: ignore already expired entries.
        today = pd.Timestamp.utcnow().tz_localize(None).normalize()
        future_expiries = []
        for expiry in expiries:
            dt = pd.to_datetime(expiry, errors='coerce')
            if pd.notna(dt) and dt.normalize() >= today:
                future_expiries.append((dt.normalize(), expiry))

        if future_expiries:
            future_expiries.sort(key=lambda x: x[0])
            expiries = [x[1] for x in future_expiries[:max_expiries]]
        else:
        ...
        return (
            float(ratio),
            True,
            f'{ticker} Options ({used} Expiries)'
        )

    except Exception:
        return np.nan, False, 'Option chain unavailable'
...
def fetch_sp500_pe_history():
    ...
    headers = {
        "User-Agent": (
            "Mozilla/5.0 "
            "(compatible; QuantRegimeDashboard/4.0)"
        )
    }

    # Preferred path: parse the actual static HTML table.
    try:
        tables = pd.read_html(
            requests.get(
                url,
    ...
    # Fallback: simple row extraction if pandas' HTML parser
...
def fetch_multi_asset_data(selected_asset):
    ...
    # IMPORTANT:
    # DX-Y.NYB is the Yahoo Finance spot US Dollar Index.
    # DX=F is not used as the primary DXY feed anymore.
    tickers = {
        'asset': cfg['ticker'],
    ...
    asset_ok = (
        ...
        and not close["asset"].dropna().empty
    )
    ...
    if not has_volume:
        # Do not manufacture zero volume. A zero-filled fallback would
        # create artificial OBV data and could make a missing feed look valid.
        asset_vol = pd.Series(
            np.nan,
            index=price.index,
            dtype=float
    ...
    if FRED_API_KEY:
        try:
            ...
            if (
                ...
                # FRED units are explicit:
                # WALCL / WTREGEN = millions of USD
                # RRPONTSYD = billions of USD
                # Convert the first two to billions before subtraction.
    ...
    if cot is not None and not cot.empty:
        df["cot_commercials"] = safe_reindex_series(
            cot,
            df.index
        )
    else:
        df["cot_commercials"] = np.nan
    ...
    if isinstance(fg, pd.Series) and not fg.empty:
        df["fear_greed"] = safe_reindex_series(
            fg,
            df.index
        )
    else:
    ...
    if selected_asset == "S&P 500":
        ...
        if pe_series is not None and not pe_series.empty:
            df["pe_valuation"] = safe_reindex_series(
                pe_series,
                df.index
            )
        else:
    ...
    for pillar, weights in active.items():
        ...
        dash[f"Saeule_{pillar}"] = (
            score_series
        )

        dash[f"Coverage_{pillar}"] = (
            coverage_series
        )

        pillar_coverages[pillar] = (
            coverage_series
        )
    ...
    # Model coverage across the six active pillars.
    coverage_cols = [
        f"Coverage_{p}"
        for p in cfg["Saeulen_Gewichte"]
        if f"Coverage_{p}" in dash.columns
        and cfg["Saeulen_Gewichte"].get(p, 0) > 0
    ]
...
st.caption(
    ...
    "wie viel der aktiv gewichteten "
    "Indikator-Gewichte tatsächlich mit Daten befüllt sind."
)
...
if score >= 60:
    ...
    bias = "🟡 NEUTRAL / RANGE"
    rule = (
        "Keine klare Trendrichtung. Nur selektive Setups."
    )
    pos = "50% Size"
...
saeulen_details = {
    ...
    "Fundamentale_Faktoren": {
        ...
        "funktion": (
            ...
            "Für Assets ohne aktive Fundamental-Säule "
            "ist die Gewichtung 0 %."
        ),
        "links": [
            (
...
execution_data_ok = (
    model_confidence >= 65
    and today.Model_Data_Coverage >= 60
)
...
with st.expander(
    ...
    for i, (asset_name, rule_cfg) in enumerate(
        ASSET_RULES.items()
    ):
...
with st.expander(
    ...
    for i, (feed, status_value) in enumerate(
        feed_status.items()
    ):
        sc[i % 2].markdown(
            f"**{feed}:** "
    ...
    st.caption(
        "Fallback-/Offline-Komponenten werden neutral "
        "behandelt und reduzieren die ausgewiesene "
        "Datenabdeckung. Sie gelten nicht als Live-Daten."
    )
