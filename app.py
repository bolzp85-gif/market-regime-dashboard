...
# Konfiguration für dynamische Lookback-Fenster (in Handelstagen)
LOOKBACK_CONFIG = {
    "fed_policy": 1260,      # ~5 Jahre
    "real_yields": 756,      # ~3 Jahre
    "net_liquidity": 756,    # ~3 Jahre
    "credit_spreads": 756,   # ~3 Jahre 
    "usd_index": 504         # ~2 Jahre
    # Alle anderen nutzen automatisch 252 Tage
}
...
def normalize_to_percentile(series: pd.Series, lookback: int = 252, invert: bool = False) -> pd.Series:
    ...
    # Fix: Fester min_periods=20 verhindert NaNs bei lückenhafter Historie
...
# 🔑 FRED API-KEY SICHER LADE-LOGIK
FRED_API_KEY = ""
try:
...
def fetch_real_market_data():
    ...
    # Fix: Wilder's RSI (Exponentiell geglättet für präzise TradingView-Werte)
    delta = sp500.diff()
    ...
    return df_dashboard.round(1).dropna(), feed_status
...
# Fix: Schutz vor komplett leerem Datensatz (verhindert IndexError)
if df.empty:
...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("Tipp: Die Makro-Säule berechnet Zinszyklen nun dynamisch auf Basis von bis zu 5 Jahren (1260 Handelstage) für maximale Präzision.")
...
if score >= 60:
    bias, rule = "🟢 BULLISCH (Long Bias)", "Suche bevorzugt nach Dip-Käufen an Intraday-Support-Zonen (VWAP, EMA 20). Short-Setups meiden."
    pos_size = "100% Standardsize" if mci >= 70 else "75% Size (MCI uneinig)"
elif score <= 40:
    bias, rule = "🔴 BÄRISCH (Short Bias)", "Suche bevorzugt nach Short-Setups bei Erholungen an Intraday-Resistance-Zonen. Longs nur als Scalps."
    pos_size = "100% Standardsize" if mci >= 70 else "75% Size (MCI uneinig)"
else:
    bias, rule = "🟡 NEUTRAL / RANGE", "Der Markt hat keine klare Richtung. Handle Range-Setups an den Grenzen der Initial Balance."
