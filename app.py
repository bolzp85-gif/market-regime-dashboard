...
def fetch_option_put_call(ticker, max_expiries=3):
    """
    Aggregiertes Put/Call-Volumenverhältnis der nächsten Optionslaufzeiten.

    Primär wird yfinance.Ticker.option_chain() verwendet.
    Der direkte Yahoo-JSON-Endpunkt bleibt als Fallback.
    ...
    # --------------------------------------------------------
    # FALLBACK: direct Yahoo option endpoint
    # --------------------------------------------------------
...
def fetch_sp500_pe_history():
    """
    S&P-500-PE:
    1) Primär SPY-Valuation-History über yfinance
    2) Fallback Multpl
    """
    yf_pe, yf_ok = (
    ...
    headers = {
        "User-Agent": (
            "Mozilla/5.0 "
            "(compatible; QuantRegimeDashboard/5.2)"
        )
    }
