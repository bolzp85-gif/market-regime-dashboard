...
def fetch_google_trends_sentiment(asset_name):
    ...
    try:
        ...
        def z(s):
            ...
            sd = s.rolling(21, min_periods=5).std().replace(0, np.nan)
            return (s - m) / sd
    ...
    try:
        ...
        if not result:
            return (
                np.nan,
                False,
                "Yahoo option chain empty"
            )

        root = result[0]

        expiries = (
            root.get("expirationDates")
            or []
        )
        ...
        return (
            float(ratio),
            True,
            f"Yahoo Options {ticker} ({used} Expiries)"
        )
        ...
        return (
            np.nan,
            False,
            f"Yahoo options error: {str(exc)[:90]}"
        )
...
def fetch_sp500_pe_history():
    ...
    try:
        ...
        for date_text, value_text in pattern.findall(
            r.text
        ):
            try:
                dt = pd.to_datetime(
...
with st.sidebar:
    st.markdown(
        ...
        f"**Google Trends Retail Sentiment**"
        f"{' *(Fallback / Offline)*' if not trends_live else ''}"
    )
...
saeulen_details = {
    ...
    "Positionierung": {
        ...
        "funktion": (
            "Spekulative institutionelle Positionierung, "
...
with st.expander(
    ...
    st.caption(
        "Fallback-/Offline-Komponenten erhalten kein eigenes "
        "Modellgewicht. Verfügbare Komponenten werden dynamisch "
        "re-gewichtet und fehlende Daten reduzieren die "
        "Model Data Coverage sowie die Model Confidence."
    )
