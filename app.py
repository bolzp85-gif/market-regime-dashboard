...
def fetch_fear_and_greed():
    ...
    # Optimierung 1: "Menschlicherer" Header gegen Bot-Blockaden von CNN
    headers = {
...
def fetch_multi_asset_data(selected_asset):
    ...
    # Optimierung 2: "DX=F" (Futures) statt "DX-Y.NYB" für den US Dollar Index
    tickers = {
...
st.title("🦅 Institutional Market Regime Dashboard")
st.markdown("Quantitative Regime-Klassifizierung basierend auf Makro, Orderflow und Positionierung. *Developed via Streamlit.*")

selected_asset = st.selectbox("🎯 Asset Klasse wählen", list(ASSET_CONFIGS.keys()), index=0)

with st.spinner(f"Lade institutionelle Marktdaten für {selected_asset}..."):
    df_dash, feed_status = fetch_multi_asset_data(selected_asset)

if df_dash.empty:
    st.error("Fehler beim Laden der Preisdaten. Yahoo Finance ist möglicherweise nicht erreichbar.")
else:
    ...
    # HERO SECTION
    # -----------------------------
    regime = get_regime_label(heute['Final_Regime_Score'])
    st.subheader(f"Aktuelles Marktsystem: {regime}")
    
    # Optimierung 3: +.1f anstatt +g für sauberes ".0" Delta Formatting
    ...
    vola = heute.get("Raw_Volatility", 20.0)
    thresh = VOLA_THRESHOLDS.get(selected_asset, 30.0)
    if vola > thresh:
        st.warning(f"⚠️ **Volatilitäts-Warnung:** Implizite Volatilität extrem hoch ({vola:.1f}). Der Regime Score könnte verzögert reagieren. Positionsgröße reduzieren.")
