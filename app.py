...
def fetch_google_trends_sentiment(asset_name):
    """Google Trends sentiment with TrendSpy first and pytrends fallback.

    TrendSpy is used because the original pytrends project is archived and
    its old session/bootstrap logic is increasingly prone to HTTP 429.
    The dashboard treats Trends as supplemental data; a failed request never
    becomes a fake neutral live value.
    """
...
def fetch_cot_data(asset_search_string, market_code=None):
    """Fetch CFTC Legacy Futures-Only COT via the official Socrata API.

    The positioning signal is based on:
        Non-Commercial Long - Non-Commercial Short

    A COT observation is accepted only when its report date is no more
    than CFTC_MAX_AGE_DAYS old. Stale/missing COT data is treated as a
    failed feed, not as a neutral 50 score.
    """
...
def fetch_option_put_call(ticker, max_expiries=3):
    """Aggregate option-volume Put/Call ratio from Yahoo's option endpoint.

    This bypasses the fragile ``Ticker.options`` wrapper and reads the same
    Yahoo option-chain endpoint used by current yfinance versions.
    """
    ...
    try:
        ...
        # Nearest non-expired expiries, maximum max_expiries.
...
def fetch_sp500_pe_history():
    ...
    headers = {
        "User-Agent": (
            "Mozilla/5.0 "
            "(compatible; QuantRegimeDashboard/5.0)"
        )
    }
...
def fetch_multi_asset_data(selected_asset):
    ...
    cot, cot_ok, cot_source = fetch_cot_data(
        cfg["cot_code"],
        cfg.get("cot_market_code")
    )
    ...
    if cot is not None and not cot.empty:
        df["cot_noncommercials"] = safe_reindex_series(
            cot,
            df.index
        )
    else:
    ...
    # CFTC is a hard requirement for the Positionierung pillar.
    # If CFTC is unavailable/stale, do not manufacture a neutral 50.
    # The pillar is removed from the final weighted score and the remaining
    # active pillar weights are renormalized row-by-row.
    ...
    # Row-wise weighted score: unavailable hard-gated pillars are excluded
    # and the remaining weights are renormalized. This prevents missing CFTC
    # data from becoming an artificial neutral contribution.
