...
def fetch_fear_and_greed():
    """Holt den echten CNN Fear & Greed Index Score und Historie."""
...
def fetch_put_call_ratio():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            s = df.dropna(subset=['Date', 'Ratio']).set_index('Date')['Ratio'].sort_index()
            if not s.empty:
                return s
    except Exception:
        pass

    # Alternativer Versuch via yfinance Ticker
    try:
        pc_df = yf.download("^PCRATIO", period="1y", progress=False)["Close"]
        if not pc_df.empty:
            return pc_df.dropna()
    except Exception:
        pass
...
def fetch_fundamental_metrics():
    """Holt das echte KGV und Gewinnwachstum für den S&P 500 via SPY ETF."""
    pe_val = 24.5
    eg_val = 8.0
    try:
        spy = yf.Ticker("SPY")
...
def fetch_sp500_cot_data():
    ...
    if frames:
        try:
            ...
            df_all['Date'] = pd.to_datetime(df_all[date_col].astype(str), format='%Y%m%d', errors='coerce')
            df_all['Net_Commercials'] = pd.to_numeric(df_all[long_col], errors='coerce') - pd.to_numeric(df_all[short_col], errors='coerce')
            
            s = df_all.dropna(subset=['Date']).set_index('Date')['Net_Commercials'].sort_index()
...
# ==========================================
# 4. MAIN PIPELINE (ALL LIVE DATA)
# ==========================================

@st.cache_data(ttl=3600)
def fetch_real_market_data():
    """Aggregiert 100% echte Datenquellen aus Yahoo, FRED, CFTC, CNN & CBOE."""
    tickers = {
        "sp500": "^GSPC",
        "vix": "^VIX",
        "dxy": "DX-Y.NYB",
    ...
    if FRED_API_KEY and FRED_API_KEY != "DEIN_FRED_API_KEY_HIER":
        try:
            ...
            df_raw["fed_policy"] = fed_funds.reindex(df_raw.index, method='ffill').ffill().bfill()
            df_raw["real_yields"] = real_yields.reindex(df_raw.index, method='ffill').ffill().bfill()
        except Exception:
    ...
    if cot_series is not None and not cot_series.empty:
        df_raw["cot_commercials"] = cot_series.reindex(df_raw.index, method='ffill').ffill().bfill()
    else:
    ...
    if isinstance(fg_data, pd.Series) and not fg_data.empty:
        df_raw["fear_greed"] = fg_data.reindex(df_raw.index, method='ffill').ffill().bfill()
    ...
    if pc_series is not None and not pc_series.empty:
        df_raw["put_call_ratio"] = pc_series.reindex(df_raw.index, method='ffill').ffill().bfill()
    else:
        # Fallback auf geglätteten VIX-Proxy falls CBOE geblockt wird
        df_raw["put_call_ratio"] = (df_raw["vix_score"] / 30.0).clip(0.4, 1.2)

    # Fundamental Valuation (P/E & Growth)
...
# Daten laden
with st.spinner('Lade 100% reale Live-Daten (Yahoo, FRED, CFTC, CNN & CBOE)...'):
...
st.caption(f"Stand: {df.index[-1].strftime('%d.%m.%Y')} | Asset: S&P 500 (100% Echtzeit-Schnittstellen)")
st.markdown("---")
...
st.markdown("---")
st.markdown("### 📈 Historischer Verlauf (100% Live-Daten)")
