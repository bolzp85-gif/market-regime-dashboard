...
def fetch_cot_data(asset_search_string, market_code=None):
    """Fetch CFTC Legacy Futures-Only COT via the official public API.

    The previous implementation depended on annual ZIP files and fuzzy name
    matching. The official CFTC Socrata API is more stable for a dashboard:
    use the exact CFTC market code and request only the recent observations.
    """
    ...
    try:
        ...
        if date_col is None or long_col is None or short_col is None:
            return pd.Series(dtype=float), False, "CFTC fields changed"
        ...
        return (
            d.set_index("Date")["Net_Commercials"],
            True,
            f"CFTC API / {code}"
        )
