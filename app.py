...
ASSET_CONFIGS = {
    ...
    "EUR/USD": {
        ...
        # Kein VIX/VXN/GVZ/OVX als künstlicher Proxy.
        # Die Volatilität wird direkt aus EUR/USD berechnet.
        ...
        # Bei EUR/USD ist ein stärkerer USD grundsätzlich
        # Gegenwind für EUR/USD.
        "invert_inverts": [
            "vix_score",
            "usd_index",
            "fed_policy",
        ...
        # EUR/USD reagiert besonders stark auf Makro/Zinsumfeld
        # und technischen Trend.
        "Saeulen_Gewichte": {
            "Makroekonomie": 0.35,
            "Positionierung": 0.20,
            "Marktinterna": 0.10,
            "Technischer_Trend": 0.20,
            "Fundamentale_Faktoren": 0.05,
            "Fruehwarnindikatoren": 0.10
        },
...
# ============================================================
# 2. BASISGEWICHTE DER SUB-INDIKATOREN
# ============================================================
...
VOLA_THRESHOLDS = {
    ...
    # Bei EUR/USD:
    # annualisierte 20-Tage historische Volatilität
    # in Prozent.
    "EUR/USD": 20.0
}
...
def fetch_google_trends_sentiment(asset_name: str):
    ...
    try:
        ...
        valid_bull = [
            kw for kw in bull_kws
            if kw in df_trends.columns
        ]

        valid_bear = [
            kw for kw in bear_kws
            if kw in df_trends.columns
        ]
        ...
        latest_spread = float(
            net_spread.dropna().iloc[-1]
        )

        # Kontraindikator:
        # Euphorie -> niedriger Score
        # Panik -> hoher Score

        contrarian_score = float(
            np.clip(
                50.0 - (latest_spread * 15.0),
                0.0,
                100.0
            )
        )
        ...
        return (
            round(contrarian_score, 1),
            round(latest_spread, 2),
            True
        )
...
def normalize_to_percentile(
    ...
    z_scores = (
        clean_series - rolling_mean
    ) / rolling_std

    percentiles = (
        norm.cdf(z_scores) * 100
    )
...
def calculate_mci(
    ...
    gesamt_score = np.average(
        scores,
        weights=weights
    )

    weighted_variance = np.average(
        (scores - gesamt_score) ** 2,
        weights=weights
    )
...
def fetch_cot_data(
    ...
    for yr in years:
        ...
        url = (
            f"https://www.cftc.gov/files/"
            f"dea/history/fut_com_txt_{yr}.zip"
        )
...
def fetch_multi_asset_data(
    ...
    # Volatilitätsindex nur dann hinzufügen,
    # wenn das Asset tatsächlich einen eigenen
    # Volatilitätsindex besitzt.

    if cfg["volatility_ticker"] is not None:
    ...
    # --------------------------------------------------------
    # Volatilitätsindex / Volatilität
    # --------------------------------------------------------

    if (
        cfg["volatility_type"]
        == "index"
        and "vix" in close_data.columns
    ):
        ...
        # EUR/USD:
        # 20-Tage historische annualisierte
        # Volatilität aus Tagesrenditen.
        ...
        # Für die Marktinterna brauchen wir weiterhin
        # einen Volatilitätsindikator.
        #
        # Für EUR/USD verwenden wir dafür die
        # historische Volatilität selbst.
    ...
    # --------------------------------------------------------
    # Marktbreite / Momentum Proxy
    #
    # Hinweis:
    # Bei Einzelassets ist dies keine echte
    ...
    if selected_asset == "S&P 500":
        ...
        # Bewusster statischer Proxy wie im bisherigen Modell.
        df_raw[
            "pe_valuation"
        ] = 24.5
    ...
    if FRED_API_KEY:
        ...
        try:
            ...
            walcl_s = safe_reindex_series(
                fred.get_series("WALCL"),
                df_raw.index
            )

            tga_s = safe_reindex_series(
                fred.get_series("WTREGEN"),
                df_raw.index
            )

            rrp_s = safe_reindex_series(
                fred.get_series("RPTCW"),
                df_raw.index
            )
    ...
    cot_reindexed = safe_reindex_series(
        cot_data,
        df_raw.index
    )
...
vola_limit = VOLA_THRESHOLDS.get(
    selected_asset,
    30.0
)

cfg_current = ASSET_CONFIGS[
    selected_asset
]
...
if current_vola >= vola_limit:
    ...
    st.error(
        ...
        f"Extremes Slippage-Risiko! "
        f"**Regel: Kein Handel oder "
        f"maximal 25% Positionsgröße!**"
    )
    ...
    st.warning(
        ...
        f"{vola_ticker} bei "
        f"**{current_vola:.2f}**. "
        f"Achte auf saubere Entries und "
        f"verkleinere ggf. die Positionsgröße."
    )


# ============================================================
# 21. INTRADAY TRADING BIAS
# ============================================================
...
if score >= 60:
    ...
    rule = (
        f"Suche bevorzugt nach "
        f"Dip-Käufen bei "
        f"{selected_asset} an "
        f"dynamischen Support-Zonen "
        f"(VWAP / EMAs)."
    )
    ...
    rule = (
        ...
        "Handle Vor-Börsen-Extrema "
        "oder Range-Rotationen."
    )
...
st.caption(
    "Ein unabhängiger Kontraindikator "
    "basierend auf dem Suchverhalten "
    "von Privatanlegern."
)
...
with col_gt2:
    ...
    if contra_score >= 65:
        ...
        st.success(
            "🟢 **Panik-Ausschlag "
            "(Boden-Aussicht):** "
            "Angst-Überhang im Retail-"
            "Segment. Potenzielles "
            "Kauf-Setup."
        )
        ...
        st.info(
            "🟡 **Ausgeglichenes "
            "Sentiment:** "
            "Keines der Extrema erreicht."
        )


with col_gt3:

    cfg_trends = TREND_KEYWORD_MAP.get(
        selected_asset,
        TREND_KEYWORD_MAP["S&P 500"]
    )
...
saeulen_details = {
    ...
    "Makroekonomie": {
        ...
        "funktion":
            "Zinsumfeld, Zentralbank-"
            "Liquidität & Dollar-Stärke.",
    ...
    "Positionierung": {
        ...
        "quelle":
            "CFTC CoT-Report & CNN Fear & Greed",
...
# ============================================================
# 25. INTRADAY EXECUTION CHECKLIST
# ============================================================
...
with col_c1:
    ...
    c1_val = st.checkbox(
        "Trendkonformität "
        "(Preis über relevanten "
        "EMAs / Marktstruktur intakt)",
        value=trend_intakt,
        key="chk_trend_det"
    )
...
if (
    ...
    st.success(
        ...
        "Alle Filter grün. Setup "
        "entspricht dem definierten "
        "Market Regime und den "
        "Risikoparametern."
    )
    ...
    st.warning(
        "🟡 **CAUTION / WARNUNG:** "
        "Gemischte Signale. "
        "Execution nur mit reduzierter "
        "Positionsgröße oder an "
        "exakten charttechnischen "
        "Extrempunkten."
    )
...
with st.expander(
    ...
    st.caption(
        "Das Dashboard nutzt intelligente "
        "Fallbacks. Ein Ausfall einzelner "
        "APIs führt nicht zum Absturz, "
        "sondern aktiviert statistische "
        "Proxies."
    )